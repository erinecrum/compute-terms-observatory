...
def _comparison_sheet(ws, dataset: dict) -> None:
    providers = dataset["providers"]
    dims = dataset["dimensions"]
    matrix = dataset["matrix"]

    ws.title = "Comparison"
    ws["A1"] = "Compute Contract Terms Observatory"
    ...
    for d in dims:
        ...
        for c, p in enumerate(providers, start=3):
            ...
            ws.cell(row=r, column=c, value=f.get("value", ""))
...
def write_workbook(dataset: dict, path: str | Path) -> Path:
    wb = Workbook()
    _comparison_sheet(wb.active, dataset)
